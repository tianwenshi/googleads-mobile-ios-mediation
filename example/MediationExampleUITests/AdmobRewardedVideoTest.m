...
@interface AdmobRewardedVideoTest : XCTestCase
@property (nonatomic, strong) XCUIApplication *app;

@end

@implementation AdmobRewardedVideoTest
...
-(void)testLoadRewardedVideo {
    ...
    XCUIElement *show = self.app.buttons[@"Show Rewarded Ad"];
    !show.exists?:[show tap];
    sleep(10);
}

-(void)testLoadRewardedVideoClosed {
    XCUIElement *Label = self.app.staticTexts[@"Objective-C Custom Event"];
    XCTAssertTrue(Label.exists, @"Objective-C Custom Event not found!");
    [Label tap];
    sleep(10);
    ...
    sleep(35);
    XCUIElement *close = self.app.buttons[@"ad_closeBtn"];
    if(close.exists) {
       [close tap];
    }
...
-(void)testLoadRewardedVideoClicked {
    XCUIElement *Label = self.app.staticTexts[@"Objective-C Custom Event"];
    XCTAssertTrue(Label.exists, @"Objective-C Custom Event not found!");
    [Label tap];
    sleep(10);
